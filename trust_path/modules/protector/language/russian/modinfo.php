...
define($constpref."_DESC","Этот модуль защищает Ваш сайт от различного вида атак, таких как: DoS, SQL Injection и порчи переменных.");
...
define($constpref.'_ADMENU_MYBLOCKSADMIN','Права доступа') ;
...
define($constpref.'_DEFAULT_LANGDSC','Установите язык, на котором будут отображаться сообщения об ошибках, возникших до выполнения common.php');

define($constpref.'_RELIABLE_IPS','Доверенные адреса');
define($constpref.'_RELIABLE_IPSDSC','Установите адреса, при заходе с которых проверки безопасности не проводятся. Разделяйте каждый адрес знаком "|". "^" соответствует началу строки, "$" соответствует концу строки.');

define($constpref.'_LOG_LEVEL','Журнал событий');
define($constpref.'_LOG_LEVELDSC','');

define($constpref.'_BANIP_TIME0','Время приостановки запрещенного (забанненого) IP (сек)');
...
define($constpref.'_HIJACK_TOPBITDSC','Анти-Налет Сеанса:<br />Значение по умолчанию 32 (бит).  (Все биты защищены)<br />Когда ваш IP не устойчив, установите диапазон IP адресов числом битов.<br />(пример) Если Ваш IP находится в диапазоне 192.168.0.0-192.168.0.255, установите 24 (бит) здесь');
...
define($constpref.'_CONTAMI_ACTIONDS','Выберите действие, выполняемое в случае когда кто-либо пытается передать вашему скрипту "грязные" системные переменные. (Рекомендуется: пустой экран)');
...
define($constpref.'_BF_COUNTDSC','Установите максимальное количество попыток входа пользователя за 10 минут. В случае если кто-либо попытается залогиниться большее чем указано количество раз - его адрес будет занесен в черный список (RBL).');

define($constpref.'_BWLIMIT_COUNT','Ограничение ширины канала');
define($constpref.'_BWLIMIT_COUNTDSC','Определите максимальное количество обращений к mainfile.php за время наблюдения. Это значение должно быть от 0 для CPU, имеющих достаточную производительность канала. Значение более 10 будет игнорировано.');
...
define($constpref.'_OPT_BIPTIME0','Запрещение (бан) IP (moratorium)');
...
define($constpref.'_DOSOPT_BIPTIME0','Запрещение (бан) IP (moratorium)');
define($constpref.'_DOSOPT_HTA','Запретить доступ используя .htaccess (экспериментально)');

define($constpref.'_BIP_EXCEPT','Группы пользователей никогда не попадающие в черный список.');
define($constpref.'_BIP_EXCEPTDSC','Рекомендуется всегда добавлять в этот список группу Администраторов сайта.');

define($constpref.'_DISABLES','Деактивировать потенциально опасные функции системы');
